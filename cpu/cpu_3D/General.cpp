...
void write_to_xyz(std::ofstream& xyz,std::vector<Cell>& subcells,const Cell& wholecell) // write to xyz file
{
 ...
 for(int i=0;i<size;i++)
 {
  ...
  xyz<<pro[j].getsymbol()<<" "<<(pro[j].getposition()).getx()+((wholecell.getbreadth()+1)*subcells[i].get_colno())<<" "<<(pro[j].getposition()).gety()+((wholecell.getlength()+1)*subcells[i].get_rowno())<<" "<<(pro[j].getposition()).getz()<<"\n";
 }
}
