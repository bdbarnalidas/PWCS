...
void Cell::addprotein(const Cell& obj,const int n,const int seed,const Simulate& sim) // add proteins to cell and initializing the parameters of protein
{
 ...
 for(int i=0;i<n;i++)
 {
  b=((double) (rand() % 99 + 1) / 100.0) * obj.getlength();
  a=((double) (rand() % 99 + 1) / 100.0) * obj.getbreadth();
  ...
  Protein p(symbol,1,0.5,position,velocity,halfvelocity,force); // create a protein
